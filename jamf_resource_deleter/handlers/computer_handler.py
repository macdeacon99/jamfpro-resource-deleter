...
class ComputerHandler(ResourceHandler):
    ...
    # TODO - Tidy up to make it only required resources
    ...
    def _json_to_jamf_computer_xml_dicttoxml(self, config_data):
        ...
        if "purchasing" in computer_data:
            ...
            if "is_purchased" in purchasing:
                clean_data["purchasing"]["is_purchased"] = str(
                    purchasing["is_purchased"]
                ).lower()
            if "is_leased" in purchasing:
                clean_data["purchasing"]["is_leased"] = str(
        ...
        if "security" in computer_data:
            ...
            if "activation_lock" in security:
                clean_data["security"]["activation_lock"] = str(
                    security["activation_lock"]
                ).lower()
            if "recovery_lock_enabled" in security:
                clean_data["security"]["recovery_lock_enabled"] = str(
                    security["recovery_lock_enabled"]
                ).lower()
            if "firewall_enabled" in security:
                clean_data["security"]["firewall_enabled"] = str(
        ...
        if (
            "extension_attributes" in computer_data
            and computer_data["extension_attributes"]
        ):
        ...
        if "groups_accounts" in computer_data:
            ...
            if "computer_group_memberships" in ga and ga["computer_group_memberships"]:
                clean_data["groups_accounts"]["computer_group_memberships"] = ga[
