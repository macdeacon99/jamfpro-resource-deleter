    ...
    def _json_to_jamf_profile_xml_dicttoxml(self, config_data):
        ...
        if 'general' in profile_data:
            ...
            if 'distribution_method' in general:
                clean_data['general']['distribution_method'] = general['distribution_method']
            if 'user_removable' in general:
                clean_data['general']['user_removable'] = str(general['user_removable']).lower()
            if 'level' in general:
                clean_data['general']['level'] = general['level']
            if 'uuid' in general:
                clean_data['general']['uuid'] = general['uuid']
            if 'redeploy_on_update' in general:
                clean_data['general']['redeploy_on_update'] = general['redeploy_on_update']
        ...
        if 'scope' in profile_data:
            ...
            if 'all_computers' in scope:
                clean_data['scope']['all_computers'] = str(scope['all_computers']).lower()
            if 'all_jss_users' in scope:
                clean_data['scope']['all_jss_users'] = str(scope['all_jss_users']).lower()
            
            # Handle arrays in scope
            for key in ['computers', 'buildings', 'departments', 'computer_groups', 
            ...
            if 'limitations' in scope:
                ...
                for key in ['users', 'user_groups', 'network_segments', 'ibeacons']:
                    if key in scope['limitations'] and scope['limitations'][key]:
                        clean_data['scope']['limitations'][key] = scope['limitations'][key]
            ...
            if 'exclusions' in scope:
                ...
                for key in ['computers', 'buildings', 'departments', 'computer_groups',
                    ...
                    if key in scope['exclusions'] and scope['exclusions'][key]:
                        clean_data['scope']['exclusions'][key] = scope['exclusions'][key]
        ...
        if 'self_service' in profile_data:
            ...
            if 'self_service_display_name' in ss:
                clean_data['self_service']['self_service_display_name'] = ss['self_service_display_name']
            if 'install_button_text' in ss:
                clean_data['self_service']['install_button_text'] = ss['install_button_text']
            if 'self_service_description' in ss:
                clean_data['self_service']['self_service_description'] = ss['self_service_description']
            if 'force_users_to_view_description' in ss:
                clean_data['self_service']['force_users_to_view_description'] = str(ss['force_users_to_view_description']).lower()
            
            if 'security' in ss:
                clean_data['self_service']['security'] = {}
                if 'removal_disallowed' in ss['security']:
                    clean_data['self_service']['security']['removal_disallowed'] = ss['security']['removal_disallowed']
            
            if 'self_service_icon' in ss and ss['self_service_icon']:
                clean_data['self_service']['self_service_icon'] = ss['self_service_icon']
            
            if 'feature_on_main_page' in ss:
                clean_data['self_service']['feature_on_main_page'] = str(ss['feature_on_main_page']).lower()
            
            if 'self_service_categories' in ss and ss['self_service_categories']:
                clean_data['self_service']['self_service_categories'] = ss['self_service_categories']
            
            if 'notification' in ss:
                clean_data['self_service']['notification'] = ss['notification']
            if 'notification_subject' in ss:
                clean_data['self_service']['notification_subject'] = ss['notification_subject']
            if 'notification_message' in ss:
                clean_data['self_service']['notification_message'] = ss['notification_message']
